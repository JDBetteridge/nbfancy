...
def html(args):
    ...
    for infile in contents:
        ...
        if args.include_slides:
            slides = nbftools.notebook2slides(
                os.path.join(args.input_dir, infile))
    ...
    redirect_dir = pkg_resources.resource_filename(resource_package,
                                                   redirect_path)
...
def main():
    ...
    parser.add_argument('verb',
                        choices=['init', 'hello', 'configure',
                                 'rerun', 'render', 'html'],
                        help='''action to perform.
Try adding --help to one of these options for more usage information''')
